...
function lerProdutos(){ // Função para ler o arquivo produtos.json
    const dados = fs.readFileSync('./data/produtos.json', 'utf-8');

    return JSON.parse(dados);
};

router.get('/', (req, res) => {  //Rota GET (ler) todos os produtos
    const produtos = lerProdutos();
    res.json(produtos);
});

router.get('/:id', (req, res) => {  // Rota GET (ler) produto por ID
...
router.post('/', (req, res) => { //Rota POST (adicionar) ao registro de produtos
